...
from lvae.datasets import make_trainloader
...
def parse_args():
    ...
    parser.add_argument('--load_optim', action='store_true', default=False) # action=argparse.BooleanOptionalAction
    # data setting
    parser.add_argument('--trainset',   type=str,  default='coco-train2017')
    parser.add_argument('--transform',  type=str,  default=None)   # 'crop=256,hflip=True'
    ...
    parser.add_argument('--eval_first', action='store_true', default=False) # action=argparse.BooleanOptionalAction
    # exponential moving averaging (EMA)
    parser.add_argument('--ema',        action='store_false', default=True) # action=argparse.BooleanOptionalAction
    parser.add_argument('--ema_decay',  type=float,default=0.9999)
    parser.add_argument('--ema_warmup', type=int,  default=10_000)
    # automatic mixed precision (AMP)
    parser.add_argument('--amp',        action='store_true', default=False)   # argparse.BooleanOptionalAction
    # device setting
    parser.add_argument('--fixseed',    action='store_false', default=True) # action=argparse.BooleanOptionalAction
    ...
    cfg.model_log_interval = 2000        #2000
...
class TrainWrapper(BaseTrainingWrapper):
    def set_dataset(self):
        ...
        trainset = get_hsi_dateset(cfg.trainset, transform_cfg=cfg.transform)
    ...
    def evaluate(self):
        ...
        # results_to_log = process_log_results(results, cfg.valset)  # 比较和VTM的压缩对比
        ...
        mean_loss = sum(results['loss'])/len(results['loss'])
        msg = self.stats_table.get_body() + '||' + '%10.4g' * 1 % (mean_loss)
...
def read_rd_stats_from_json(json_path):     # 用于和别的模型比较，计算db rate
...
def get_anchor_stats(dataset_name):     # 用于和别的模型比较，计算db rate
    anchor_paths = {
...
def compute_bd_rate_over_anchor(stats, dataset_name):   # 用于和别的模型比较，计算db rate
