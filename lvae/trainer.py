...
class BaseTrainingWrapper():
    ...
    def _save_if_best(self, checkpoint):
        ...
        mean_loss = sum(cur_loss) / len(cur_loss)
        if isinstance(self._best_loss, list):
            self._best_loss = sum(self._best_loss) / len(self._best_loss)
        if mean_loss < self._best_loss:
            self._best_loss = mean_loss
            svpath = self._log_dir / 'best.pt'
            torch.save(checkpoint, svpath)
            logging.info(f'Get best loss = {mean_loss}. Saved to {svpath}.')
