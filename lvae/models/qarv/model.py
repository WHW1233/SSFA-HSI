...
class VariableRateLossyVAE(nn.Module):
    ...
    def forward_end2end(self, im: torch.Tensor, lmb: torch.Tensor, mode='trainval', get_latent=False):
        ...
        if getattr(self, 'adapters', None) is not None:
            for key, adapter in self.adapters.items():
                if key in enc_features:
                    enc_features[key] = adapter(enc_features[key])
        nB, _, xH, xW = x.shape
    ...
    def forward(self, batch, lmb=None, return_rec=False):
        ...
        loss = kl/2 + lmb * distortion    # kl为平均每个band的损失
        loss = loss.mean(0)
        ...
        with torch.no_grad():
            # for training print
            stats['bppix'] = kl.mean(0).item() * self.log2_e
    ...
    def study(self, save_dir, **kwargs):
        ...
        for k in [1, 2]:
            ...
            if im_samples.shape[1] > 3:
                selected_indices = [5, 13, 20]
                im_samples = im_samples[:, selected_indices, :, :]
            tv.utils.save_image(im_samples, fp=save_path, nrow=math.ceil(num**0.5))
        # reconstructions    是否在训练过程中重构图像
    ...
    def _self_evaluate(self, img_paths, lmb: float, pbar=False, log_dir=None):
        ...
        for impath in pbar:
            if str(impath).endswith('.npy'):
                img = np.load(impath)
                if img.shape[1] > 100:
                    img = img[10:40, :, :]
                img = np.transpose(img, (1,2,0))
                imgh, imgw = img.shape[:2]
                img_padded = coding.nppad_divisible_by(img, div=self.max_stride)
            else:
    ...
    def compress_file(self, img_path, output_path, lmb=None):
        ...
        header_str = struct.pack('2H', imh, imw)
        # save bits to file
