...
@register_model
def qarv_hsi(lmb_range=(32,8190), pretrained=False):
    cfg = dict()
    ...
    # model configuration
    ch =  256 #128
    enc_dims =  [256, ch*3, ch*4, ch*4, ch*4]  #[192, ch*3, ch*4, ch*4, ch*4]
    ...
    dec_dims = [ch*4, ch*4, ch*3, ch*2, ch*1] # [ch*4, ch*4, ch*3, ch*2, ch*1]
    z_dims =  [32*8, 32*8, 96*8, 8*8] # [32, 32, 96, 8]
    cfg['dec_blocks'] = [
        # 1x1
...
@register_model
def qarv_hsi_lower(lmb_range=(32,4096), pretrained=False, adapted=False):
    cfg = dict()
    ...
    # model configuration
    ch = 256  # 128
    enc_dims = [192, ch*2, ch*3, ch*3, ch*3]  # [192, ch*3, ch*4, ch*4, ch*4]
    ...
    cfg['enc_blocks'] = [
        # 1x1卷积实现降维
        # nn.Conv2d(im_channels, channel_1, kernel_size=1),  # 1x1卷积，降维到channel_1通道
        # nn.LeakyReLU(),
        # nn.Conv2d(channel_1, channel_2, kernel_size=1),  # 再次1x1卷积，降维到channel_2通道
        # 64x64
        common.patch_downsample(channel_2, enc_dims[0], rate=2),
        # 16x16
        *[res_block(enc_dims[0], kernel_size=7) for _ in range(6)],
        res_block(enc_dims[0]),
        common.patch_downsample(enc_dims[0], enc_dims[1]),
    ...
    dec_dims = [ch*3, ch*3, ch*3, ch*2, ch*1] # [ch*4, ch*4, ch*3, ch*2, ch*1]
    z_dims =  [32*4, 32*4, 96*4, 8*4] # [32, 32, 96, 8]
    cfg['dec_blocks'] = [
        # 1x1
        *[qarv.VRLVBlockBase(dec_dims[0], z_dims[0], enc_key='enc_s64', enc_width=enc_dims[-1], kernel_size=1, mlp_ratio=4) for _ in range(1)],
        res_block(dec_dims[0], kernel_size=1, mlp_ratio=4),
        ...
        *[qarv.VRLVBlockBase(dec_dims[3], z_dims[3], enc_key='enc_s8', enc_width=enc_dims[-4], kernel_size=7, mlp_ratio=1.75) for _ in range(3)],
        ...
        common.patch_upsample(dec_dims[3], dec_dims[4], rate=2),
        # 16x16
        *[res_block(dec_dims[4], kernel_size=7, mlp_ratio=1.5) for _ in range(7)],
        common.patch_upsample(dec_dims[4], channel_2, rate=2),

        #光谱维度升维
        # nn.Conv2d(channel_2, channel_1, kernel_size=1),  # 1x1卷积，升维到channel_1通道
        # nn.LeakyReLU(),
        # nn.Conv2d(channel_1, im_channels, kernel_size=1),  # 再次1x1卷积，升维到输入通道
    ]
    ...
    if adapted:
        from lvae.models.qarv.adapters import SSFMA
        # Freeze base parameters
        for p in model.parameters():
            p.requires_grad = False
        
        # Instantiate and register sequential SSFMA adapters
        adapters = nn.ModuleDict({
            'enc_s8': SSFMA(channels=512),
            'enc_s16': SSFMA(channels=768),
            'enc_s32': SSFMA(channels=768),
            'enc_s64': SSFMA(channels=768),
        })
        model.adapters = adapters
        
        # Unfreeze only the adapters
...
@register_model
def qarv_hsi_higher(lmb_range=(16,2048), pretrained=False):
    cfg = dict()
    ...
    # model configuration
    ch = 256  # 128
    enc_dims = [192, ch*3, ch*4, ch*4, ch*4]  # [192, ch*3, ch*4, ch*4, ch*4]
    ...
    dec_dims = [ch*4, ch*4, ch*4, ch*2, ch*1] # [ch*4, ch*4, ch*3, ch*2, ch*1]
    z_dims =  [32*8, 32*8, 96*8, 8*8] # [32, 32, 96, 8]
    cfg['dec_blocks'] = [
        # 1x1
        *[qarv.VRLVBlockBase(dec_dims[0], z_dims[0], enc_key='enc_s64', enc_width=enc_dims[-1], kernel_size=1, mlp_ratio=4) for _ in range(1)],
