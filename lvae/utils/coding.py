...
def nppad_divisible_by(img, div=64):
    """ Pad a numpy.ndarray such that both its sides are divisible by `div`
    ...
    if (h_old % div == 0) and (w_old % div == 0):
        return img
    h_tgt = div * ((h_old + div - 1) // div)
    w_tgt = div * ((w_old + div - 1) // div)
    pad_width = ((0, h_tgt - h_old), (0, w_tgt - w_old)) + ((0, 0),) * (img.ndim - 2)
